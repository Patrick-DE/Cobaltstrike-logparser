...
def run(args):
    ...
    if args.path:
        ...
        with futures.ThreadPoolExecutor(max_workers=args.worker) as e:
            # parse all logs
            result_futures = list(map(lambda file: e.submit(parse_beacon_log, file), log_files))
            result_futures += list(map(lambda file: e.submit(parse_add_log, file), ev_files))
            result_futures += list(map(lambda file: e.submit(parse_add_log, file), dl_files))
            ...
            result_futures = list(map(lambda beacon: e.submit(fill_beacon_info, beacon), incpl_beacons))
            for idx, future in enumerate(futures.as_completed(result_futures)):
